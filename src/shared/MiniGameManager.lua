--[[
    MiniGameManager.lua
    by Jules (v5 - Formatted and Corrected)
...
function MiniGameManager.startQTE()
    ...
    for r = 1, 3 do
        for c = 1, 3 do
            ...
            button.Position = UDim2.new(0, 85 + (c - 1) * 100, 0, 50 + (r - 1) * 100)
    ...
    coroutine.wrap(function()
        while currentRound <= roundsToWin and not success and not isInterrupted() do
            local sequence = {}
            for i = 1, currentRound + 2 do table.insert(sequence, math.random(#buttons)) end
            task.wait(1)
        ...
        card.MouseButton1Click:Connect(function()
            ...
            if not firstCard then
                ...
                if firstCard.id == secondCard.id then
                    ...
                    if pairsFound == 6 then success = true end
                else
...
function MiniGameManager.init()
    ...
    if not machinesFolder:FindFirstChild("MiniGameMachine") then
        local machine = Instance.new("Part", machinesFolder)
        machine.Name = "MiniGameMachine"
        machine.Size = Vector3.new(4, 6, 2)
        machine.Position = Vector3.new(10, 3, 10)
        machine.Anchored = true
        machine.BrickColor = BrickColor.new("New Yeller")
    ...
    RunService.RenderStepped:Connect(function()
        ...
        if not character or not character.PrimaryPart then interactionPrompt.Enabled = false; return end
